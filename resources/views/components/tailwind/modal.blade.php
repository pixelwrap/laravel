<div id="{{$modal->id}}" data-modal-target="{{$modal->id}}" @if($modal->static) data-modal-backdrop="static" @endif tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
    <div class="relative w-full max-h-full {{$modal->sizeClasses}}">
        <div class="relative bg-gray-200 {{$modal->roundClasses ?? ''}} shadow-sm dark:bg-gray-700">
            <div class="flex items-center justify-between px-4 py-4 md:px-5 md:py-3 border-b dark:border-gray-600 border-gray-300">
                ...
                @if($modal->showClose)
                    <button type="button" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 {{$modal->roundClasses ?? ''}} text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white" data-modal-hide="{{$modal->id}}">
