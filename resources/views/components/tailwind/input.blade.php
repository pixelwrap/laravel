@php
  ...
  if(($input->classType ?? "float") === "default") {
  ...
  $props = filterAndMapObjectProps($input, ['id', 'name', 'label', 'fieldType', 'type', 'required']);
...
@else
    ...
    @if($floatInputClass === $inputClass)
        <div class="relative z-0 w-full group">
            <input {{$props}} id="{{$inputId}}" type="{{$inputType}}" name="{{ $inputId }}" value="{{ $inputValue }}"
                   placeholder=" " class="{{$inputClass}}" @required($inputRequired)/>
