<div class="relative z-0 w-full group">
    ...
    @else
        <div class="relative w-full">
            ...
            @endif
            <input @disabled($input->disabled) type="{{$input->fieldType}}"
                   placeholder="{{$input->placeholder(get_defined_vars())}}" class="{{$input->classes}}"
                   @required($input->required) value="{{$input->value(get_defined_vars())}}" name="{{$input->name}}"
                   autocomplete="{{ $input->autocomplete === true ? "on" : "new-password" }}" id="{{$input->id}}"/>
