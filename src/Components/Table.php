...
class Table extends Listing
{
    ...
    public function setDataset($args): void {}

    public function parseProps($table, $data): void
    {
        $table->nodes = [];
        if (isset($table->dataset)) {
            if (is_array($table->dataset)) {
                $datasets = [];
                $names = $table->dataset;
        ...
        if (is_object($fields)) {
            ...
            foreach ($fields as $index => $field) {
                if (!isset($field->key) || !isset($field->label)) {
                    $this->errors[] = sprintf(
                        "Key and Label for field %s must be set. Please check if your template is compliant with the specification.",
                        $index + 1
                    );
                } else {
                            ...
                            if (in_array($aggregate, array_keys($allowed))) {
                                if (is_array($this->dataset)) {
                                    foreach ($this->dataset as $datasetIndex => $dataset) {
                                        $this->aggregate[$datasetIndex][$allowed[$aggregate]][$field->key] = collect($dataset)
                                            ->map(fn($d) => $d->toArray())
                                            ->{$aggregate}($field->key);
                                    }
                                } else {
                                    $this->aggregate[][$allowed[$aggregate]][$field->key] = collect($this->dataset)
                                        ->map(fn($d) => $d->toArray())
                                        ->{$aggregate}($field->key);
                                }
                            } else {
                                $this->errors[] = sprintf(
                                    "Aggregated must be one of %s. Please check if your template is compliant with the specification.",
                                    implode(", ", $allowed)
                                );
                            }
                        }
        ...
        if ($this->variant === "primary") {
            ...
            $this->addClass(
                "odd:bg-gray-50 odd:dark:bg-gray-900 even:bg-gray-100 even:dark:bg-gray-700 border-b dark:border-gray-700 border-gray-200",
                "rowClasses"
            );
        } elseif ($this->variant === "secondary") {
